...
const STATES = ['default', 'typing', 'filled', 'error']
const STATE_LABEL = {
  default: 'Default',
  typing:  'Typing',
  filled:  'Filled',
...
const STATE_VALUES = {
  default: '',
  typing:  '',
  filled:  'I want to dispute this transaction because the merchant charged me twice.',
  error:   '',
}
...
function StatesSection() {
  return (
    <DocCard>
      ...
      <CardBody>
        {STATES.map(state => (
          <div key={state} style={{ display: 'flex', flexDirection: 'column', alignItems: 'flex-start', gap: 8, minWidth: 280 }}>
            <TextareaInput
              state={state}
...
// ─── Main TextareaInputDocs component ─────────────────────────────────────────

export default function TextareaInputDocs({ comp }) {
  const nodeId = comp?.id ?? '5529:781'
  return (
