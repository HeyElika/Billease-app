...
const BEHAVIOR_RULES = [
  ...
  ['It goes the way the parent went',
   'Forward, the old content leaves to the left and the new arrives from the right. Back mirrors it. The direction is read from the selection, so the handover continues the movement that caused it.'],
  ...
  ['The frame holds still',
   'Headings, labels and any controls that are the same across selections keep their place. What changes is the part that actually belongs to one selection.'],
...
const SPEC_ROWS = [
  ...
  ['End to end',       `${M.holdMs + M.enterDelay + M.enterMs}ms from the commit`],
  ['What is animated', 'The content, as one element. Never a row, a label, a value or an icon.'],
  ['Viewport',         'Clipped, so nothing escapes the region. Never a visible edge, and never a wipe.'],
